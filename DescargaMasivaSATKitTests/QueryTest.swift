...
class StubQuerySharedSession: SharedSession {
  ...
  func data(for: URLRequest) async throws -> (Data, URLResponse) {
    ...
    let urlResponse = HTTPURLResponse(
      url: URL(
        string: "https://cfdidescargamasivasolicitud.clouda.sat.gob.mx/SolicitaDescargaService.svc")!,
      statusCode: statusCode, httpVersion: "1.1",
      headerFields: [
        "Content-Encoding": "gzip", "Content-Length": "357",
        "Content-Type": "text/xml; charset=utf-8", "Date": "Thu, 11 Sep 2025 05:29:59 GMT",
...
final class QueryTest: XCTestCase {
  ...
  override func setUp() async throws {
    if let certUrl = Bundle(for: QueryTest.self).url(
      forResource: "certificate", withExtension: "cer"),
      let keyURL = Bundle(for: QueryTest.self).url(forResource: "privkey", withExtension: "key")
    {
  ...
  func test404Request() async {
    ...
    params.endPoint = .facturas
    let request = QueryEndpoint(params: params)
    await XCTAssertThrowsErrorAsync(try await request.request(sharedSession)) { error in
      XCTAssertEqual(
        error as? QueryEndpointResultError,
        QueryEndpointResultError
