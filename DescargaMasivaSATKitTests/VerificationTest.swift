...
final class VerificationTest: XCTestCase {
  ...
  override func setUp() async throws {
    if let certUrl = Bundle(for: VerificationTest.self).url(
      forResource: "certificate", withExtension: "cer"),
      let keyURL = Bundle(for: VerificationTest.self).url(
        forResource: "privkey", withExtension: "key")
    {
  ...
  func test404Request() async {
    ...
    let verification = VerificationEndpoint(queryId: "e3e2636f-c008-405b-a36c-205baabd9297")
    await XCTAssertThrowsErrorAsync(try await verification.request(sharedSession)) {
      error in
      XCTAssertEqual(
        error as? VerificationEndpointResultError,
        VerificationEndpointResultError
