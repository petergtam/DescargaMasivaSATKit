...
struct StubAuthenticationSharedSession: SharedSession {
  ...
  func data(for: URLRequest) async throws -> (Data, URLResponse) {
    ...
    let urlResponse = HTTPURLResponse(
      ...
      httpVersion: "1.1",
      headerFields: [
        "Content-Encoding": "gzip", "Content-Length": "801",
        "Content-Type": "text/xml; charset=utf-8",
        "Date": "Thu, 11 Sep 2025 05:29:59 GMT", "Server": "Microsoft-IIS/10.0",
...
final class Authentication: XCTestCase {
  ...
  func loadCertificates() async throws {
  ...
  func testAddCertData() async throws {
    if let certUrl = Bundle(for: Authentication.self).url(
      forResource: "certificate",
      withExtension: ".cer"
    ),
      let keyUrl = Bundle(for: Authentication.self).url(
        forResource: "privkey",
        withExtension: ".key"
      )
    {
      ...
      try await AuthenticationManager.shared.add(
        certData: certData,
        keyData: keyData,
  ...
  func testAddCertUtils() async throws {
    if let certUrl = Bundle(for: Authentication.self).url(
      forResource: "certificate",
      withExtension: ".cer"
    ),
  ...
  func test404Request() async throws {
    ...
    await XCTAssertThrowsErrorAsync(
      try await AuthenticationManager.shared.getToken(sharedSession)
    ) {
      error in
      XCTAssertEqual(
        error as? AuthenticationResultError,
        AuthenticationResultError
          .httpError(
            statusCode: 404,
            contents: [
              "2025-09-12T03:55:32.573Z",
              "2025-09-12T04:00:32.573Z",
