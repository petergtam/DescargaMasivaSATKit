...
public struct QueryEndpoint {
  ...
  private func createSolicitaDescargaBody() throws -> String {
    ...
    let digestInfo =
      "<\(endPoint) xmlns=\"http://DescargaMasivaTerceros.sat.gob.mx\">\(nodoSolicitud)</\(endPoint)>"
  ...
  func request(_ sharedSession: SharedSession = URLSession.shared) async throws -> String {
    ...
    guard
      let url = URL(
        string:
          "https://\(params.isRetention ? "reten": "cfdi")descargamasivasolicitud.clouda.sat.gob.mx/SolicitaDescargaService.svc"
      )
    else {
    ...
    request.addValue(
      "http://DescargaMasivaTerceros.sat.gob.mx/ISolicitaDescargaService/\(endPoint)",
      forHTTPHeaderField: "SOAPAction")
    request.addValue("text/xml; charset=utf-8", forHTTPHeaderField: "Content-Type")
    request.addValue(
      "WRAP access_token=\"\(tokenData.token)\"", forHTTPHeaderField: "Authorization")
