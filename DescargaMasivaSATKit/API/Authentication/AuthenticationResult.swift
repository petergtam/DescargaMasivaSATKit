...
struct AuthenticationResultError: Error, Equatable {
  ...
  var localizedDescription: String {
    switch code {
    case .dateParsingFailed(let contents):
      return "Failed to parse date from contents: \(contents)"
    case .httpError(let statusCode, let contents):
...
class AuthenticationResult: NSObject {
  ...
  init(
    ...
    completionHandler: @escaping (Date?, Date?, String?, Error?) -> Void
  ) {
    self.data = data
...
extension AuthenticationResult: XMLParserDelegate {
  ...
  func parserDidEndDocument(_ parser: XMLParser) {
    if hasError {
      completionHandler(
        nil, nil, nil,
        AuthenticationResultError.httpError(statusCode: response?.statusCode, contents: contents))
    } else {
      let formatter = ISO8601DateFormatter()
      formatter.formatOptions = [.withInternetDateTime, .withFractionalSeconds]
      guard let created = formatter.date(from: contents[0]),
        let expires = formatter.date(from: contents[1])
      else {
        completionHandler(
          nil, nil, nil, AuthenticationResultError.dateParsingFailed(contents: contents))
        return
      }
