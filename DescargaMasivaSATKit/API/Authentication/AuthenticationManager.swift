...
struct AuthenticationManagerError: Error {
  ...
  static var authenticationBodyFailed: AuthenticationManagerError {
    .init(code: .authenticationBodyFailed)
  }
...
public class AuthenticationManager {
  ...
  public func add(certData: Data, keyData: Data) async throws {
    try await add(certData: certData, keyData: keyData, service: CryptoService())
  }

  func add(certData: Data, keyData: Data, service: Cryptobable = CryptoService()) async throws {
    self.certUtils = try await CertUtils(certData: certData, keyData: keyData, service: service)
  }
  ...
  func getToken(
    _ sharedSession: SharedSession = URLSession.shared, _ now: Date = Date.now,
    isRetention: Bool = false
  ...
  private func getCFDIAuthenticationToken(
    _ sharedSession: SharedSession = URLSession.shared, _ now: Date = Date.now
  ) async throws -> TokenData {
  ...
  private func getRetenAuthenticationToken(
    _ sharedSession: SharedSession = URLSession.shared, _ now: Date = Date.now
  ) async throws -> TokenData {
  ...
  private func createAuthenticationBody() throws -> String {
    ...
    let created = now.utc
    let timestamp =
      "<u:Timestamp xmlns:u=\"http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-utility-1.0.xsd\" u:Id=\"_0\"><u:Created>\(created)</u:Created></u:Timestamp>"
  ...
  private func createTokenData(
    _ sharedSession: SharedSession = URLSession.shared, isRetention: Bool = false
  ) async throws -> TokenData {
    ...
    guard
      let url = URL(
        string:
          "https://\(isRetention ? "reten" : "cfdi")descargamasivasolicitud.clouda.sat.gob.mx/Autenticacion/Autenticacion.svc"
      )
    else {
    ...
    return try await withCheckedThrowingContinuation { continuation in
      let autResult = AuthenticationResult(data: data, response: response) {
        (created, expires, token, error) in
        if let error {
          continuation.resume(throwing: error)
        }
        if let created, let expires, let token {
          continuation.resume(
            returning: TokenData(created: created, expires: expires, token: token))
        }
      }
